...
def clean_llm_text(text: str) -> str:
    ...
    cleaned_lines = [
        line
        for line in lines
        if not line.startswith("▶️ Տրեյլեր") and not line.startswith("🎞️ Դիտելու հղում")
    ]
    return "\n".join(cleaned_lines)
        ...
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="🎞 Տրեյլեր", url=trailer_url or "https://youtube.com"
                )
            ],
            [
                InlineKeyboardButton(
                    text="🌐 Դիտել", url=watch_url or "https://example.com"
                )
            ],
        ]
    )
...
async def send_series_blocks(text: str, message: Message):
    ...
    for chunk in series_chunks:
        ...
        await message.answer(
            pretty_text, parse_mode="HTML", reply_markup=inline_keyboard
        )
...
async def handle_series_genre(message: Message, state: FSMContext):
    ...
    if not genre:
        await message.answer(
            ...
            reply_markup=series_genre_menu,
        )
        return
    ...
    await message.answer(
        "🤖 Կարող ես ընտրել այլ ժանր կամ վերադառնալ մենյու։",
        reply_markup=series_genre_menu,
    )
    await state.set_state(SeriesStates.waiting_for_genre)
